...
CudaRasterizer::TileState CudaRasterizer::TileState::fromChunk(char*& chunk, size_t N)
{
  ...
  cub::DeviceScan::InclusiveSum(nullptr, tile.tile_scan_size, tile.tile_point_touched, tile.tile_point_touched, N);
  obtain(chunk, tile.tile_scanning_space, N, 128);

  CHECK_CUDA(cudaMemset(tile.tile_counters, 0, N * sizeof(uint32_t)), debug);
	...
	bool debug)
{
  ...
  */
  CHECK_CUDA(cub::DeviceScan::InclusiveSum(tileState.tile_scanning_space,tileState.tile_scan_size,tileState.tile_point_touched,tileState.tile_offsets,Tiles_Num), debug)
